...
javascript:(()=>{
  ...
  ui.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:4px">
      ...
      <span id="bdg" style="padding:2px 8px;border-radius:999px;background:#10b981;color:#052;font-weight:700;font-size:10px;letter-spacing:.25px">RUN</span>
    </div>
    ...
    <div style="margin-top:10px;border-top:1px solid rgba(255,255,255,.16);padding-top:8px;font-size:11px">
      블라이, '상위노출' 분석 서비스 :
  ...
  const L={follow:'팔로우',surf:'파도타기',paused:'일시정지',done:'100회 완료'};
  const render=r=>{ST.textContent=(r?r+' | ':'')+`${L.follow}:${S.totalFollow} | ${L.surf}:${S.totalSurf}`};
  ...
  const setPaused=(r=L.paused)=>{S.run=false;pBtn.textContent='Resume';setBtnColor(pBtn,C.blue);
                                 pBtn.style.width='38%';
                                 cBtn.style.display='inline-block';setBtnColor(cBtn,C.redDim);BDG.textContent='휴식중';
                                 BDG.style.background='#f59e0b';render(r)};
  ...
  const loop=setInterval(async()=>{
    ...
    if(S.totalSurf>=100){setPaused(L.done);return}
    if(S.empty>=3){
