(()=>{
  ...
  const S={
    ...
    totalFollow:0,
    totalSurf:0,
    clickedFollows:new Set(), // 이미 팔로우한 버튼 기록
  ...
  cBtn.onclick=()=>{try{clearInterval(loop)}catch{};try{clearInterval(scroller)}catch{};try{obs&&obs.disconnect()}catch{};
                    window.__followBot=false;try{ui.remove()}catch{}};
  ...
  async function clickAllFollows(){
    ...
    for(const b of findFollowBtns()){
      const id=extractIdFromEl(b.closest('li')) || txt(b);
  ...
  function randomProfileCandidate(){
    ...
    const items=QQ(listSel).filter(vis);
    const candidates=[];
  ...
  const loop=setInterval(async()=>{
    ...
    if(S.totalSurf>0 && S.totalSurf%100===0){setPaused();return} // 100배수 Stop
    if(S.empty>=5){ // 5회 연속 팔로우 없음
