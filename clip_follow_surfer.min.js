(()=>{
  ...
  const S={run:true,delay:650,scrollDelay:900,empty:0,scrolls:0,totalFollow:0,totalSurf:0,
           ...
           clickedFollows:new Set()}; // ✅ 중복 카운트 방지용
  ...
  function extractIdFromEl(el){
    ...
    if(!id&&el){const t=(el.innerText||el.textContent||'').trim();const mt=t.match(/@([A-Za-z0-9._-]{3,32})/);if(mt)id=mt[1]}
  ...
  // ✅ 중복 카운트 방지: 이미 처리한 엘리먼트/프로필 제외
  function findFollowBtns(){
    return QQ('button,a,[role="button"]').filter(b=>{
      const t=txt(b);
      if(b.dataset.followCounted==='1') return false;
      const id=extractIdFromEl(b.closest('li')||b);
      if(id && S.clickedFollows.has(id)) return false;
      return /팔로우/.test(t) && !/팔로잉|팔로우중|Following/i.test(t) &&
             !b.closest('[disabled],[aria-disabled="true"]') && vis(b)
    });
  }

  // ✅ 클릭 후 엘리먼트/프로필 둘 다 마킹
  async function clickAllFollows(){
    let n=0;
    for(const b of findFollowBtns()){
      const id=extractIdFromEl(b.closest('li')||b) || null;
      if(id && S.clickedFollows.has(id)) continue;
