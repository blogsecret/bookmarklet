(()=>{
  ...
  const S={run:true,delay:650,scrollDelay:900,empty:0,scrolls:0,totalFollow:0,totalSurf:0,
           visited:new Set(),clickedFollows:new Set()};
  ...
  ui.innerHTML=`
    ...
    <div id="btnRow" style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;justify-content:center">
      ...
      <button id="c" style="flex:1 1 auto;width:38%;padding:4px 10px;border:0;border-radius:10px;font-weight:700;cursor:pointer;box-shadow:0 1px 0 rgba(255,255,255,.07) inset,0 2px 10px rgba(0,0,0,.25);display:none">Close</button>
    </div>
  ...
  async function clickAllFollows(){
    ...
    for(const b of findFollowBtns()){
      const id = extractIdFromEl(b.closest('li')) || b.textContent.trim();
      if(S.clickedFollows.has(id)) continue;
  ...
  const loop=setInterval(async()=>{
    ...
    if(S.totalSurf>0 && S.totalSurf % 100 === 0){ // 100회 배수마다 Stop
      setPaused();return;
    }
