(()=>{
  ...
  const S={run:true,delay:650,scrollDelay:900,empty:0,scrolls:0,totalFollow:0,totalSurf:0,
           ...
           busyClicking:false}; // ✅ 클릭 중 스크롤 차단
  ...
  function findFollowBtns(){
    return QQ('a[role="button"],button,[role="button"]').filter(b=>{
      if(b.dataset.__cf_clicked) return false;
      const t=txt(b);
      const isFollow=/팔로우/.test(t) && !/팔로잉|팔로우중|Following/i.test(t);
      const notPressed=b.getAttribute('aria-pressed')!=='true';
      const notDisabled=!b.closest('[disabled],[aria-disabled="true"]');
      return isFollow && notPressed && notDisabled && vis(b);
    });
  }

  // ✅ 보이는 모든 팔로우 버튼을 다 클릭한 뒤에만 반환 (최대 3 패스)
  async function clickAllFollows(){
    S.busyClicking=true;
    try{
      let totalClicked=0;
      for(let pass=0; pass<3; pass++){
        const btns=findFollowBtns();
        if(!btns.length) break;
        for(const b of btns){
          const now=(b.innerText||b.textContent||'').trim();
          if(/팔로잉|팔로우중|Following/i.test(now)) { b.dataset.__cf_clicked='1'; continue; }

          // 계정 ID 기반 중복 카운트 방지
          const id = extractIdFromEl(b.closest('li')) || (b.getAttribute && b.getAttribute('href')) || now;
          if(id && S.clickedFollows.has(id)) { b.dataset.__cf_clicked='1'; continue; }
          if(id) S.clickedFollows.add(id);
          ...
          totalClicked++; S.totalFollow++; render();
          await sleep(S.delay);
        }
        // 버튼 상태가 "팔로우중"으로 바뀌며 새 버튼이 뚫릴 수 있어 짧게 대기 후 재검색
        await sleep(150);
      }
      S.empty = totalClicked ? 0 : S.empty + 1;
      return totalClicked;
    } finally {
      S.busyClicking=false;
    }
  }
  ...
  function extractIdFromEl(el){
    ...
    if(!id&&el){
      const idNode=el.querySelector && el.querySelector('[class*="UIProfileListItem_txt_area"],[class*="UIProfileListItem_tit_area"]');
  ...
  const scroller=setInterval(()=>{
    if(!S.run||S.busyClicking) return;            // ✅ 클릭 중엔 스크롤 금지
  ...
  const loop=setInterval(async()=>{
    ...
    // ✅ 현재 화면에 있는 팔로우 버튼을 전부 소진할 때까지 클릭
    const clicked = await clickAllFollows();
    render();

    // 100회 배수 Stop
    if(S.totalSurf>0 && S.totalSurf % 100 === 0){
      setPaused();return;
    }
    // 팔로우 없음 5회 연속 → 파도타기
    if(S.empty>=5){
