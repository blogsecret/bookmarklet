(()=>{
  ...
  // --- State & Colors -------------------------------------------------------
  const S={
    run:true, delay:650, scrollDelay:900, empty:0, scrolls:0,
    totalFollow:0, totalSurf:0,
    visited:new Set((()=>{try{return JSON.parse(localStorage.getItem('__clipVisited__')||'[]')}catch{return[]}})()),
    followedIds:new Set(),          // ✅ 이미 팔로우 처리한 핸들(세션 중복 방지)
    clickedBtns:new WeakSet()       // ✅ 같은 엘리먼트 중복 클릭 방지(보조)
  };
  ...
  ui.innerHTML=`
    <div style="margin-bottom:10px;border-bottom:1px solid rgba(255,255,255,.16);padding-bottom:8px;font-size:11px">
      블라이, '상위노출' 분석 서비스 :
      <a href="https://blai.co.kr?invite=%EB%8A%91%EB%8C%80%EC%99%95" target="_blank" rel="noopener" style="color:#9ccfff;text-decoration:underline">바로가기</a>
    </div>
  ...
  cBtn.onclick=()=>{try{clearInterval(loop)}catch{};try{clearInterval(scroller)}catch{};
                    try{obs&&obs.disconnect()}catch{};window.__followBot=false;try{ui.remove()}catch{}};
  ...
  function gotoFollowerTabIfSeen(){
    const cand=QQ('a,button,[role="button"]').find(el=>/팔로워/i.test(el.textContent||'')); // ko
  ...
  function getIdForButton(btn){
    const root = btn.closest('li,div[class*="UIProfileListItem"],[class*="UIProfileListItem"]') || btn;
    const link = root.querySelector('a[href^="/@"],a[href*="/@"]') || btn.closest('a[href^="/@"],a[href*="/@"]');
    return extractIdFromEl(link||root)
  }

  function isFollowingStateIn(rootOrBtn){
    const candidates = rootOrBtn.querySelectorAll ? rootOrBtn.querySelectorAll('button,a,[role="button"]') : [rootOrBtn];
    for(const x of candidates){
      const t=txt(x);
      if(/팔로잉|팔로우중|Following/i.test(t)) return true;
      const ap=x.getAttribute('aria-pressed'); if(ap==='true') return true;
    }
    return false;
  }

  async function waitForFollowState(btn, ms=2000){
    const root = btn.closest('li,div[class*="UIProfileListItem"],[class*="UIProfileListItem"]') || document;
    const end = Date.now()+ms;
    while(Date.now()<end){
      if(isFollowingStateIn(btn) || isFollowingStateIn(root)) return true;
      await sleep(120);
    }
    return false;
  }

  // --- Find follow buttons (dedup) ------------------------------------------
  function findFollowBtns(){
    const raw = QQ('button,a,[role="button"]').filter(b=>{
      const t=txt(b);
      return /팔로우/.test(t) && !/팔로잉|팔로우중|Following/i.test(t) &&
             !b.closest('[disabled],[aria-disabled="true"]') && vis(b)
    });

    // 1) 핸들(ID) 기준으로 우선 중복 제거
    const seenId=new Set();
    const byId=[];
    for(const b of raw){
      const id=getIdForButton(b);
      if(id){
    ...
    // 2) ID를 못 구한 것들은 같은 항목(루트) 중복 제거
    const idLess = raw.filter(b=>!getIdForButton(b));
    const seenRoot=new WeakSet();
    const uniqRoot = idLess.filter(b=>{
      const root=b.closest('li,div[class*="UIProfileListItem"],[class*="UIProfileListItem"]')||b;
      if(seenRoot.has(root)) return false;
      seenRoot.add(root); return true;
    });

    return [...byId, ...uniqRoot];
  }

  // --- Click all follows (with success check & dedupe) ----------------------
  async function clickAllFollows(){
    let success=0;
    for(const b of findFollowBtns()){
      const id=getIdForButton(b);
      if(id && S.followedIds.has(id)) continue;     // 이미 처리한 사용자
      if(S.clickedBtns.has(b)) continue;            // 같은 엘리먼트 재클릭 방지
      ...
      const ok = await waitForFollowState(b, 2000);
      if(ok){
        success++;
        S.totalFollow++;
        if(id) S.followedIds.add(id);
      }else{
        // 실패/지연 시 재시도 가능하도록 표시 해제
        S.clickedBtns.delete(b);
      }
      await sleep(S.delay);
    }
    S.empty = success ? 0 : S.empty+1;
    return success;
  }

  // --- Surfing to next profile ----------------------------------------------
  ...
  async function ensureMoreCandidates(){
    ...
    if(items.length<=3){
      scrollTo(0,document.body.scrollHeight); await sleep(700);
      scrollBy(0,-Math.floor(innerHeight*0.5)); await sleep(300);
      scrollTo(0,0); await sleep(400);
    }
  ...
  async function surfNextProfile(){
    ...
    if(!cand){await ensureMoreCandidates(); cand=nextProfileCandidate()}
    if(!cand) return false;
    S.visited.add(cand.id); S.totalSurf++; save();
    cand.link.scrollIntoView({block:'center'});
    cand.link.click(); await sleep(500);
  ...
  // --- Main loop with reentry guard (fixes double-count) ---------------------
  let ticking=false;
  const loop=setInterval(async()=>{
    if(!S.run || ticking) return;
    ticking=true;
    try{
      await clickAllFollows();
      render();
      if(S.totalSurf>=100){ setPaused(); return; }
      if(S.empty>=3){
        if(await surfNextProfile()){
          justNavigated=true; setTimeout(()=>justNavigated=false,1200);
